...
$response = array();
if(isset($data['token']) && isset($data['OrderID']))
{
	...
	try
	{
        ...
        $core=new Core();
         ...
         $baseUrl = "https://unitedhealthlumina.com/uhladmin/uhl-management/bookedtest/bookedreport/";

        $sql = "SELECT `ID`, `FileName` FROM `order_report` WHERE `OrderID` = '$OrderID'";
        $records = $core->_getRecords($conn, $sql);
        if (!empty($records)) {
            foreach ($records as &$record) {
                $record['DownloadLink'] = $baseUrl . $record['FileName'];
            }
            $response['error'] = false;
            $response['data'] = $records;
        } else {
            $response['error'] = true;
            $response['message'] = "No reports found for the given OrderID";
        }
