...
$response = array();
if(isset($data['token']) && isset($data['CompanyID']) && isset($data['CompanySiteID']) && isset($data['ServiceID']) && isset($data['ChannelPartner']))
{
	$jwt = $data['token'];
	try
	{
		...
		$response = $service_report_obj->InitiateServiceReport($data);
	}
	catch (Exception $e) {
	    // Invalid token
	    $response['error'] = true;
