...
/// -----new js for new form --------------------------------------------------------
                ...
                function toggleTestSelection(testID) {
                    // Get the selected test card and check mark
                    let selectedCard = document.getElementById('test-card-' + testID);
                    let checkMark = document.getElementById('check-mark-' + testID);
                    let checkbox = document.getElementById('test-' + testID);
                    ...
                    // Save the selected plans (supporting multiple selections)
                          ...
                          $('#gotothirdstep').hide();
                            async function saveSelectedTests() 
                            {
                                ...
                                let form_action = $("#form_action_test").val();
                                let form_id = $("#form_id_test").val();

                                // alert(form_action);
                                // alert(form_id);
                                // Get selected plans
                                let selectedTests = document.querySelectorAll('input[name="selected_test[]"]:checked');
                                console.log('dhuhihi',selectedTests);
                                let selectedTestIDs = Array.from(selectedTests).map(test => test.value);

                                if (selectedTestIDs.length > 0) {
                                    $("#saveTestBtn").html('Please Wait..');

                                    try {
                                        // Save the selected plans via AJAX
                                        const response = await $.ajax({
                                            url: "action/add-update-customer-test.php",
                                            type: "POST",
                                            data: {
...
function SaveTestCustomer()
{
     ...
     var PocName = $("#test_poc_name").val();
    if (PocName == "") {
        Alert("Please Enter Name");
        return false;
    }

    var PocNumber = $("#test_poc_contact_number").val();
    if (PocNumber == "") {
    ...
    let form_action = $("#form_test_action").val();
    let btntext = "";
    if (form_action == "add") {
        btntext = "Add";
    } else {
    ...
    let myForm = document.getElementById("customer_test_form");
    var formData = new FormData(myForm);
    formData.forEach(function(value, key){
    ...
    $.ajax({
        url: "action/add-update-test-customer.php",
        type: "POST",
        data: formData,
        success: function (data) {
            var response = JSON.parse(data);
...
function openTestDetailsModal(test) {
    // Ensure test data is parsed correctly
    // if (typeof test === 'string') {
    //     test = JSON.parse(test);
    // }

    // Populate the modal fields with test data
    ...
    document.getElementById('modalTestCode').textContent = test.TestCode;
    document.getElementById('modalTestFee').textContent = parseFloat(test.Fee).toLocaleString();
    document.getElementById('modalReportTime').textContent = test.ReportTime;
    document.getElementById('modalParameters').textContent = test.Parameters;
    document.getElementById('modalIdentifies').textContent = test.Identifies;
...
function showPage(page) {
    ...
    tests.forEach((test, index) => {
        test.style.display = (index >= (page - 1) * itemsPerPage && index < page * itemsPerPage) ? "block" : "none";
    });
...
function searchTests() {
    ...
    tests.forEach(test => {
        const testName = test.getAttribute('data-name');
        test.style.display = testName.includes(searchQuery) ? "block" : "none";
    });
