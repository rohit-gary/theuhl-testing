...
if(isset($customerId)){
    ...
    $Plan=new Plans($conn);
      ...
      $PolicyCustomer_details=$PolicyCustomer_detailss[0];
      ...
      $planDurationMonths = $CustomerPlanDetails['PlanDuration'];
...
<div class="recipient-details">
    <p>Mr.'.$PolicyCustomer_details['Name'].'</p>
    <p id="address">'.$formattedAddress.'</p>
    
     <p>UTTAR PRADESH  &nbsp;  '.$PolicyCustomer_details['Pincode'].'</p>
    <p>Mobile. '.$PolicyCustomer_details['ContactNumber'].'</p>
...
<div class="content">
    ...
    This is to certify that <b>'.$PolicyCustomer_details['Name'].'</b> paid RS '.$CustomerPlanDetails['PlanCost'].'.00 towards <b>'.$CustomerPlanDetails['PlanName'].'</b> on ' .$formattedDate.' Valid till '. $expiryFormattedDate.'.</p>';
   
            // Preprocess PlanHighlights to extract individual highlights
            if (!empty($CustomerPlanDetails['PlanHighlights'])) {
                // Remove any HTML tags for sanitization
                $highlightsString = strip_tags($CustomerPlanDetails['PlanHighlights']);

                // Use regex to extract quoted highlights
                preg_match_all('/"(.*?)"/', $highlightsString, $matches);

                // Extracted highlights will be in $matches[1]
                $highlights = !empty($matches[1]) ? array_map('trim', $matches[1]) : [];
            } else {
                // If PlanHighlights is empty, set highlights as an empty array
                $highlights = [];
            }

            // Generate HTML content for the highlights
            if (!empty($highlights)) {
                $html .= '<p>Following are some salient (Reimbursement UHL COIN) features of your subscription plan:</p><ul>';
                foreach ($highlights as $highlight) {
                    // Add each highlight as a list item with proper escaping
                    $html .= '<li>' . htmlspecialchars($highlight) . '</li>';
                }
                $html .= '</ul>';
            } else {
                // If no highlights are available, display a fallback message
...
<img src="' . $signature . '" alt="Signature" style="width:100px; height:40px"></div>
...
<table style="width: 100%;  border-collapse: collapse; font-size: 10px; margin-bottom: 20px;">
    ...
    <tr>
        ...
        <td style=" padding: 8px;">'.$PolicyCustomer_details['Name'].'</td>
    </tr>
    <tr>
        <td style=" padding: 8px; background-color: transparent;">Plan Name</td>
        <td style=" padding: 8px;">'.$CustomerPlanDetails['PlanName'].'</td>
    </tr>
    <tr>
...
foreach ($PolicyCustomer_Member_detailss as $index => $member) {
    ...
    $html4 .= '
    <tr>
        <th colspan="2" style=" padding: 8px; background-color: transparent; text-align: left;">Member ' . ($index + 1) . ' Details</th>
    </tr>
    <tr>
...
$html4 .= '</table>';
if (isset($CustomerPlanDetails['PlanCost']) && is_numeric($CustomerPlanDetails['PlanCost'])) {
    // Assume $CustomerPlanDetails['PlanCost'] is the gross premium
    $grossPremium = $CustomerPlanDetails['PlanCost'];
