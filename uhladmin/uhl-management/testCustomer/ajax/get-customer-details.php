...
if (isset($_POST['ID'])) {
	...
	if ($step == 3) {
		if ($PolicyNumber != "") {
			...
			$policy_amount = array();
			$where = " where PolicyNumber = '$PolicyNumber'";
			$policy_amount_records = $core->_getTableRecords($conn, 'customerpolicyamount', $where);
			if ($policy_amount_records != null) {
				$step = $step + 2;
				$policy_amount['PolicyNumber'] = $policy_amount_records[0]['PolicyNumber'];
				$policy_amount['PolicyAmount'] = $policy_amount_records[0]['Amount'];
				$_SESSION['policy_amount'] = $policy_amount;
			}
	...
	if ($step == 4) {
		if ($PolicyNumber != "") {
			...
			$where = " where PolicyNumber = '$PolicyNumber'";
			$policy_documents_records = $core->_getTableRecords($conn, 'policy_customer_documents', $where);
			if ($policy_documents_records != null) {
				$step = $step + 1;
				foreach ($policy_documents_records as $policy_documents_record) {
					array_push($policy_documents, $policy_documents_record);
				}
				$_SESSION['policy_documents'] = $policy_documents;
			}
